...
function App() {
  ...
  const fileChange = (fileID, value, key = 'title', unsaved = false, isNew = false) => {
    const currFile = files.filter(file => file.id === fileID)[0];
    // 如果当前内容和 文件内容不一样， 
    if (currFile[key] !== value) {
      // 设置当前 files
      const index = files.findIndex(file => file.id === fileID)
      const newFiles = files;
      newFiles.splice(index, 1, { ...currFile, [key]: value, isNew: false })
      setFiles(newFiles);
      // 打开文件名称更改时，同步 tab
      if (openedFileIDs.includes(fileID)) {
  ...
  const fileSearch = (name) => {
    const searchAfter = files.filter(file => file.title.includes(name) )
